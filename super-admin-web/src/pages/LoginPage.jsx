...
export default function LoginPage() {
  const [email, setEmail] = useState('admin@pharmaworld.com');
        ...
        {!showMfa ? (
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none dark:bg-gray-700 dark:border-gray-600" />
            </div>
            <div>
          ...
          <form onSubmit={handleMfa} className="space-y-4">
            ...
            <input type="text" value={mfaCode} onChange={e => setMfaCode(e.target.value)} required maxLength={6}
              className="w-full px-4 py-3 border rounded-lg text-center text-2xl tracking-[0.5em] font-bold focus:ring-2 focus:ring-primary-500 outline-none dark:bg-gray-700 dark:border-gray-600"
              placeholder="------" />
            <button type="submit" disabled={loading}
              className="w-full py-2.5 bg-primary-700 text-white rounded-lg font-medium hover:bg-primary-800 disabled:opacity-50 transition">
